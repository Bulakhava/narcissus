<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <title><?php echo (isset($title) ? $title : '404') ?></title>
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.6.1/css/all.css" integrity="sha384-gfdkjb5BdAXd+lj+gudLWI+BXq4IuLW5IT+brZEZsLFm++aCMlF1V92rMkPaX4PP" crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/css?family=Roboto:100,200,300,400,700,800" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Montserrat:100,200,300,500,600,400,700,800" rel="stylesheet">
    <link rel="stylesheet" href="/public/css/style.min.css">
    <link rel="icon" href="/img/favicons/favicon.ico" type="image/vnd.microsoft.icon" />
    <script src="/public/libraries/jquery-3.3.1.min.js"></script>
    <script src="/public/libraries/bootstrapValidator.min.js"></script>
    <link rel="stylesheet" href="/public/libraries/animate.css">
    <script src="/public/libraries/wow.min.js"></script>
    <script src="/public/js/functions.js"></script>
    <script src="/public/js/global.js"></script>
    <script src="/public/js/form.js"></script>
    <script src="/public/libraries/galleria-1.5.7.min.js"></script>

    <meta property="og:title" content="Каталог садовых цветов с фото, названиями и описанием" />
    <meta property="og:locale" content="en_RU" />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="https://narciss.by/" />
    <meta property="og:description" content="Привет, друзья! Приглашаю всех полюбоваться цветами из моего сада." />
    <meta property="og:image" content="https://narciss.by/img/og.jpg" />

    <meta name="twitter:card" content="summary">
    <meta name="twitter:site" content="Каталог садовых цветов с фото, названиями и описанием">
    <meta name="twitter:title" content="Нарциссы и другие садовые цветы">
    <meta name="twitter:description" content="Привет, друзья! Приглашаю всех полюбоваться цветами из моего сада.">
    <meta name="twitter:image" content="https://narciss.by/img/og.jpg">


    <script type="application/ld+json">
        {
            "@context":"http://schema.org",
            "@type":"Organization",
            "name":"Нарцисс и К",
            "url": "https://narciss.by/",
            "telephone":"+375296121209",
            "logo": "https://narciss.by/img/og.jpg"
        }
    </script>


    <script>
        new WOW().init({
            mobile: false
        });
    </script>


</head>
